...
=item E) Bioperl version 1.3 or higher -- L<http://www.bioperl.org>

(Note: Use anonymous CVS to get Bioperl 1.3 if it isn't yet released).
...
=head2 2. INSTALLING THE BROWSER WITH ACTIVESTATE'S PPM TOOL
...
Binary distributions of GBrowse and the 1.3 version of Bioperl live at
L<http://stein.cshl.org/ppm>.  Add this repository to the list of active
...
=head2 3. INSTALLING THE BROWSER FROM SOURCE
...
L<ORACLE_AND_BIOSQL>.  To run GBrowse on top of Gadfly, see
L<README-berkeley-gadfly>. To install on Microsoft Windows platforms, see
L<README-windows>.
...
=head2 4. POPULATING THE DATABASE (MySQL)

You can skip this step if you use the in-memory database for small
...
=head2 5a. TESTING THE BROWSER (MySQL DATABASE)
...
=head2 5b. TESTING THE BROWSER (IN-MEMORY DATABASE)

For testing the in-memory database (no MySQL server required!), this
distribution includes sample files for yeast chromosome 1.  On
Unix/Linux systems, copy the following files to /usr/tmp:

  cp sample_data/yeast1.fa  /usr/tmp
  cp sample_data/yeast1.gff /usr/tmp

You should now be able to browse yeast chromosome 1.  Type the
following URL into your favorite browser:

  http://name.of.your.host/cgi-bin/gbrowse?source=yeast1

This will display the genome browser instructions and a search field.
Type in "I" to start searching chromosome I, or search for "glutamate"
to find several genes that are involved in glutamate metabolism.

On Windows systems, copy the yeast1.fa and yeast1.gff files to
C:\Windows\Temp, and ask for this URL:

  http://name.of.your.host/cgi-bin/gbrowse?source=yeast1w

=head2 6. LOADING OTHER DATA SETS
...
=head2 7. CREATING YOUR OWN GENOME DATABASE
...
=head2 8. MAKING THE BROWSER RUN FASTER
...
=head2 9. MAKING THE BROWSER RUN SAFER
...
=head2 11. THE GBROWSE_IMG SCRIPT
...
=head2 13. THE GENBANK/EMBL PROXY
...
=head2 14. UPDATING THE BROWSER WITHOUT REENTERING YOUR DIRECTORY SETTINGS
...
=head2 16. FEATURE WISH LIST (updated June 6, 2003)
...
=head2 17. SUPPORT AND BUG REPORTS
