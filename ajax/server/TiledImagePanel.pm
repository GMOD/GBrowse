# [AVU 11/18/05]
# This is a modification of the original Bio::Graphics::Panel Perl module that
# was distributed under BioPerl 1.5.1-rc3 (?).  The goal is to make this
# module do exactly the same things that the original did, except using the
# TiledImage Perl module to intercept any calls to GD::Image and return an
# emulation of a GD::Image object that can render tiles.  This module uses
# the BioPerl framework.
#
# !!! NOTE TO SELF: I should probably change the documentation at the bottom
# when I'm through with this.


#package Bio::Graphics::TiledImagePanel;  # change if dropping into BioPerl !!!
package TiledImagePanel;

#use Bio::Graphics::BatchTiledImage;  # change if dropping into BioPerl !!!
use BatchTiledImage;
...
sub gd {
  my $self        = shift;
  #my $existing_gd = shift;  # [AVU 1/17/06] changed to pass in args;
  my %args = @_;             # we'll assume $existing_gd doesn't get used in our scheme
  ...
  #unless ($existing_gd) {  # [AVU 1/17/06] not using $existing_gd anymore
    my $image_class = $self->image_class;
    eval "require $image_class; 1" or die $@;
  #}
...
#				     );  # [AVU 11/18] old and busted use of GD::Image

#  if ($args{'
  # [AVU 11/18] we intercept the original GD::Image creation here
  my $gd = BatchTiledImage->new('-width' => $width, '-height' => $height, %args);
  ...
  $self->{gd}           = $gd;
  if ($self->bgcolor) {
      #$gd->fill(0,0,$self->bgcolor);  # [AVU 11/18] this did not work for initiating fills off-tile
      #$gd->filledRectangle(0, 0, $gd->width, $gd->height, $self->bgcolor);  # [AVU 11/18] fix for off-screen fill problem

      # [AVU 12/2]
      # the above fix was taking too long, as TiledImage had to re-run the entire command of filling
      # a massive rectangle for each tile, which is cumbersome since it takes longer to run that one
      # command than them all combined; so, we'll just many small rectangles (width 1000) instead

      # NOTE TO SELF: rectangles can be 1000 if that's the min. tile size (are we SURE of that?) or
      # perhaps using 'fill' for each tile would be better... or maybe incorporate background fill
      # into 'TiledImage' instead?  or separate background into a different tile and just do that
      # tile once, then use PNG transparency to overlay all other tiles on top of it? TINK ABOUT IT!!!

      # note that there is an extra tile at the end that is filled to accomodate elements overrunning
      # from what is really the "last" tile

#      my $tilewidth_pixels = 30000;  # !!!!!!!!!!!!!
#      for (my $offset = 0; $offset <= $width / $tilewidth_pixels; $offset++) {
#	  $gd->filledRectangle($offset * $tilewidth_pixels, 0,
#			       ($offset * $tilewidth_pixels + $tilewidth_pixels - 1), ($height - 1),
#			       $self->bgcolor);
#      }  # dude.. do NOT forget to put this back in !!!!!!!!!

  } elsif (eval {$gd->isTrueColor}) {
      $gd->fill(0,0,$translation_table{'white'});  # do we need to change this, like the above? !!!
  }
