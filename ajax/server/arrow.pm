...
#  warn join ' ',%$self;
  ...
  if ($self->option('tick')) {
    ...
    # [AVU]
    #my $units      = $self->calculate_units($start/$unit_divider,$self->feature->length/$unit_divider);  # BAD! *BAD*!!!
    my $units = $self->option('units_forced');  # because the above method for calculating units for major
                                                # ticks is foolish, we will pass the units in manually
    # [/AVU]
	...
	unless $tickpos < $left or $tickpos > $right;
      ...
      #warn "what gets printed is: $format | $scaled | $unit_label \n";  # D!!!
