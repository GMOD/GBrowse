...
sessions. GBrowse uses L<CGI::Session> to store session data on the
...
generator according to the syntax described in L<CGI::Session>. The
...
You might also want to read about L<CGI::Session::ID::salted_md5> for
...
See L<CGI.pm> for more information on the time format.
