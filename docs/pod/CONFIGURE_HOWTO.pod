...
The onload event handler will fire *after* the page is finished loading, so
this setting will be useful for running javascript functions that rely on
all or part of the HTML having been loaded and interpreted by the browser.
The onload text must use correct javascript syntax.  For example:
...
generating titles are the same as the "link" option discussed
above.The "title" option can also be computed dynamically.  See
COMPUTED OPTIONS for details.

Note HTML characters such as "<", ">" and "&" are not automatically
escaped from the title. This lets you do neat stuff, such as create
popup menus, but also means that you need to be careful. In
particular, you must not use the quote character (") in the title, but
either use the &quot; entity, or the single quote ('). The function
...
See CONFIGURE BALLOON TOOLTIPS for the ability to created rich
tooltips including images and links.
...
=head2 B12. CONFIGURE BALLOON TOOLTIPS
...
embedded query forms. See http://mckay.cshl.edu/balloons.html for
examples.
...
Alternatively, you can populate the balloon using data from an HTML
...
click option like this:

  balloon click = /cgi-bin/get_gene_data?gene=$name

In this case, when the user clicks on the feature, it creates a
...
worry about them messing up the HTML.

Customizing Balloons
--------------------

GBrowse supports multiple balloons with different shapes, sizes,
background images and timing properties. Two built-in balloons are
defined, "balloon" and "balloon_wide." The first is 308 pixels wide
and is the standard balloon that appears when for mouse click and
hover events. "Balloon_wide" is 612 pixels wide and is the one that
appears when the user clicks on the configure "[?]" icon in the
draggable track title.

To use the wide balloon rather than the standard one, format the
"balloon hover" and/or "balloon click" options like this:

  balloon click = [balloon_wide] /cgi-bin/get_gene_data?gene=$name

The [balloon_wide] keyword tells gbrowse to use the wide balloon
("balloon_wide") for clicks on these features. The standard balloon is
called "balloon", and so the following two options are equivalent:

  balloon click = /cgi-bin/get_gene_data?gene=$name
  balloon click = [balloon] /cgi-bin/get_gene_data?gene=$name

To create your own balloons, you need to create six PNG images that
describe the sides of the balloon and the connectors. To make these
images go to http://www.wormbase.org/wiki/index.php/Balloon_Tooltips
and follow the link to a CGI script that will let you design balloons
of various styles and sizes. Choose the "download all images" option
to get a tar archive containing the six images. The archive will be
named XXXXX.tar.gz, where XXXXX is a random number.

Now create a directory for the images in your web server's document
tree. We recommend placing it in $HTDOCS/gbrowse/images, where $HTDOCS
is the top of the document tree. Give the directory a good descriptive
name, such as "blue_balloon". Enter the directory and unpack the tar
archive. This will create six PNG images with names like
"down_body_XXXXX.png" where "XXXXX" is a string of numbers. Rename
each of these files to remove the _XXXXX part. Here is a complete
example:

 % cd /var/www/htdocs/gbrowse/images
 % mkdir blue_balloons
 % cd blue_balloons
 % tar zxvf ~/15855.tar.gz
   down_body_15855.png
   down_left_connector_15855.png
   down_right_connector_15855.png
   up_body_15855.png
   up_left_connector_15855.png
   up_right_connector_15855.png
 % rename _15855 '' *.png

If you do not have the rename utility on your system, you will have to
change the names manually.

Now tell GBrowse about the new balloon by adding a "custom balloons"
option to the [GENERAL] section of the config file:

 custom balloons = big_blue /gbrowse/images/blue_balloon

The format is "balloon name" "URL path to the balloon images
directory", with the two separated by whitespace. You can define
multiple custom balloons this way:

 custom balloons = big_blue /gbrowse/images/blue_balloon
                   gradient /gbrowse/images/gradient_balloon
                   psychedelic /gbrowse/images/swirly_balloon

This declares three balloons named "big_blue," "gradient" and
"psychedelic." You can now use these balloon names in track stanzas:

 [genes]
 feature = gene:curated
 glyph   = gene
 balloon hover = [gradient] <h1>Gene $name</h1> <p>$description</p>

Balloons can be customized even further with respect to their width,
height, internal padding, and the length of time they wait before
popping up. These options are described in
http://www.wormbase.org/wiki/index.php/Balloon_Tooltips#Customization.
To change the default settings, create a "balloon settings" option in
the [GENERAL] section, and configure each balloon using its name. For
example:

 [General]

 balloon settings = gradient.balloonWidth     = '308px';
                    gradient.paddingTop       = '20px';
                    gradient.paddingLeft      = '15px';
                    gradient.paddingRight     = '15px';
                    gradient.paddingBottom    = '20px';
                    gradient.paddingConnector = '48px';

You can also change the standard "balloon" and "balloon_wide"
balloons in the same way.
