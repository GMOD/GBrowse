...
This method sends a message to the server requesting that a new track be added.

The server responds with by starting to render the track and returning a track
key and the div element that will hold the track image.
