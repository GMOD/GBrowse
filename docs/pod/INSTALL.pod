...
that is via the ActiveState ppm utility.

=item Install GBrowse

The easiest way to install GBrowse on Windows is to use an
installer script:

http://wiki.gmod.org/index.php/GBrowse_Windows_HOWTO

After downloading the Windows_install.pl script, open a DOS command
shell, change directories to where the script was downloaded to,
...
=item Install the MicroSoft nmake utility 

=item Add a third party ppm repository to install GD.pm

=item Install GD.pm

=item Use CPAN tools to install other prerequisites

=item Download and install BioPerl

=item Download and install the Generic Genome Browser

=back

During the course of the install, you will prompted to answer a few 
questions; accepting the defaults is almost always the right thing to do.
...
for Macs.  The thing that generally trips up installs on Macs is getting
libgd (a prerequisite for GD.pm) installed. Some have success
...
The following arguments are recognized:
   ...
   APACHE        Base directory for the Apache conf, htdocs and cgibin directories
...
INSERT privileges.  You will need to provide the administrator name
...
These scripts are renamed to names like bp_process_gadfly.pl if you have 
chosen the "install scripts" option when you installed Bioperl.
...
Please send requests for help to <a href="mailto:gmod-gbrowse@lists.sourceforge.net">gmod-gbrowse@lists.sourceforge.net</a>.
