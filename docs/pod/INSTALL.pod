...
installer script that can be obtained from the GMOD website:
...
=item Install MicroSoft's nmake utility 

=item Add a third party ppm repository for install GD.pm
...
At the time of writing this document, release candidate 2 of 
Bioperl 1.5.2 is available on the Bioperl website, with the 
final 1.5.2 release expected any time now.
