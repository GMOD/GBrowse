...
(installed in $HTDOCS/tutorial/conf_files). Since Chado uses the
Sequence Ontology for its controlled vocabulary, it is quite likely
that this configuration file should work for any instance of Chado
once the database-specific parameters are set.  Also, depending on
what the "reference type" is (usually something like 'chromosome' or
'contig'), the line in the configuration for reference class will need
to be modified to agree with your data.
...
of this distribution, and is installed in $HTDOCS/gbrowse/tutorial/conf_files.
