...
# $Id: Browser.pm,v 1.205 2007-02-27 16:38:52 scottcain Exp $
...
sub read_configuration {
  ...
  for my $file (@conf_files) {
    ...
    next if $basename eq 'GBrowse';  # global settings
    $basename =~ s/^\d+\.//;
...
sub render_html {
  ...
  for my $label ('__scale__',@{$args->{labels}}) {
    ...
    my $img             = img({-src=>$url,-align=>'left',-usemap=>"#${label}_map",-width=>$width,
