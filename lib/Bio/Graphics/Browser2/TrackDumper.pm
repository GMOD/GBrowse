...
sub dump_sam {
    ...
    my $species = $self->data_source->species;
    my @fields;
    push @fields,"SP:$species" if $species;
    push @fields,"AS:$build"   if $build;

    for my $seqid ($db->seq_ids) {
	my $length = $db->length($seqid) or next;
	print "\@SQ\tSN:$seqid\tLN:$length\t",join("\t",@fields),"\n";
    }
...
sub dump_bigwig {
    ...
    my $build   = $self->data_source->build_id  || 'unknown';
    my $species = $self->data_source->species;
    my $taxid   = $self->data_source->taxon_id;
    my $description = "Generated by GBrowse from $origin";
    $description   .= "; species=$species" if $species;
    $description   .= "; taxid=$taxid"     if $taxid;
    $build         .= "; build=$build"     if $build;
	...
	if ($type ne $last_type or $name ne $last_name) {
	    ...
	    $name ||= $type;
	    print qq(track type=wiggle_0 name="$key ($name)" description="$description"\n);
	    $last_type = $type;
	}
...
sub print_gff3_header {
    ...
    my $source = $self->data_source;
    my $build  = $self->data_source->build_id  || 'unknown';
    my $name   = $self->data_source->species;
    my $taxid  = $self->data_source->taxon_id;

    my $ncbi   = 'http://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi';

    my $species =  $taxid ? "$ncbi?id=$taxid"
	          :$name  ? "$ncbi?name=$name"
		  :undef;
