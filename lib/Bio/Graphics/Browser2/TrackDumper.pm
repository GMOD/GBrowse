...
# Dirt simple track dumper, suitable for a lightweight replacement to DAS.
# Call this way:
...
sub print_bio_graphics_wigs {
    ...
    my $basedir = $self->data_source->code_setting($label=>'basedir');
	...
	my $l = CGI::escape($label);
	print qq(track type=wiggle_0 name="$track_name" description="Generated by GBrowse from $origin?l=$l; precision limited to 255 levels"\n);
...
sub guess_file_type {
    ...
    if ($db->isa('Bio::DB::BigFile') or $db->isa('Bio::DB::BigWigSet')) {
	return 'bed';
    }
