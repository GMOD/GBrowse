...
sub get_rich_seq {
	...
	$self->_add_genbank_seqfeature($seq,$feature,$offset,$segmentend);
    }
    return $seq;
}

sub _add_genbank_seqfeature {
    my $self = shift;
    ...
    foreach (@subf) {
	$self->_add_genbank_seqfeature($seq,$_,$offset,$length,$feature->primary_tag,$id);
    }
}
