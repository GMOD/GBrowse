...
# Field (Field, Path[, Value]) - Returns (or, if defined, sets to the new value) the specified field of a file.
sub field {
    my $self = shift;
    my $field = shift;
...
sub add_file {
    ...
    if ($self->get_file_id($path) == 0) {
		...
		warn $self->{username} . " has already uploaded $path.";
    }
}
