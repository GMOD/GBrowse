...
#  warn join ' ',%$self;
  ...
  $trunc_right = 0 if $self->no_trunc || $origin; #bizarre behavior where right arrow truncation happens when entire genome is displayed
  ...
  if ($self->option('tick')) {
    ...
    for (my $i = $origin; $i <= 2 * $last_tick; $i += $major_interval) {
      push @major_ticks, $i;  
    }
	...
	unless $tickpos < $left or $tickpos > $right;
      ...
      if ($i eq $origin && $stop >= $origin) {
	$label =~ s/[\d|.|\,]*/0/;
	$label_len = length($label) * $width;
	$middle = $tickpos - $label_len/2;
	$middle   += $interval_width if $label_intervals;
	...
	$i -= $origin;
	$label = $scale ? $i / $scale : $i;
	$scaled = $label/$divisor;
	$label = sprintf($format,$scaled,$unit_label);
	$label_len = length($label) * $width;
	$middle = $tickpos - $label_len/2;
	$middle   += $interval_width if $label_intervals;
	...
	$gd->string($font,$middle,$center+$a2-1,$label,$font_color) ##labels
	  unless ($self->option('no_tick_label') || $middle > $x2 || $i eq $origin);
      }
    }
	...
	push @minor_ticks, $i;  
      ...
      for (my $i = $origin; $i <= 2 * $last_tick; $i += $minor_interval) {
	push @minor_ticks, $i;  
      }
